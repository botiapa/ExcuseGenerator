...
module.exports = async function(app, db) {
    ...
    app.get("/add", (req, res) => {
        ...
        checkIfLoggedIn(req, res, function(profile) {
            ...
            else res.render("add_new", { profile: profile, lang: langString });
        });
    });
    app.post("/add", (req, res) => {
        checkIfLoggedIn(req, res, function(profile) {
            ...
            if (
                ...
                db.query(
                    'INSERT INTO excuses ("excuse", "lang", "added_by", "verified") VALUES($1,$2,$3,$4)', [excuse, lang, profile.ID, profile.admin],
                    function(dberr, dbres) {
                        if (!dberr) res.sendStatus(200);
    ...
    app.post("/verify", (req, res) => {
        checkIfLoggedIn(req, res, function(profile) {
            ...
            if (currentStatus == null || !ID) {
                // If one of the parameters was not satisfied
                res.sendStatus(400);
                return;
            }
            db.query(
                'UPDATE excuses SET "verified"=$1 WHERE "ID"=$2', [!currentStatus, ID],
                function(dber, dbres) {
                    if (!dber) {
                        res.sendStatus(200);
                    } else {
    ...
    app.post("/delete", (req, res) => {
        checkIfLoggedIn(req, res, function(profile) {
            ...
            if (!ID) {
                // If the parameter was not satisfied
                res.sendStatus(400);
                return;
            }
            db.query('DELETE FROM excuses WHERE "ID"=$1', [ID], function(
                dber,
                dbres
            ) {
                if (!dber) {
    ...
    app.get("/getexcuse", (req, res) => {
        getRandomExcuse("en", function(excuse) {
            if (excuse != null) res.send(excuse);
            else res.sendStatus(500);
        });
    });
            ...
            .then((response) => {
                ...
                axios({
                    ...
                    .then((response) => {
                        ...
                        db.query(
                            'SELECT * FROM users WHERE "ID"=$1', [response.data.id],
                            function(dberr, dbres) {
                                if (!dberr) {
                                    ...
                                    if (dbres.rowCount == 0) {
                                        // Not registered yet
                                        db.query(
                                            'INSERT INTO "users" ("ID", "email", "picture", "hash", "token", "refresh_token") VALUES($1, $2, $3, $4, $5, $6)', [
                                                response.data.id,
                                                response.data.email,
                                            ...
                                            function(dberr, dbres) {
                                                if (!dberr) res.cookie("hash", hash).redirect("/");
                                                else {
                                                    console.log(dberr);
                                                    res.redirect("/");
                                                }
                                        ...
                                        db.query(
                                            'UPDATE users SET "hash"=$1 WHERE "ID"=$2', [hash, response.data.id],
                                            function(dber, dbres) {
                                                if (!dberr) res.cookie("hash", hash).redirect("/");
                                                else {
                                                    console.log(dberr);
    ...
    app.post("/register", (req, res) => {
        ...
        if (username && username.length > 3 && username.length < 20 && hash != "") {
            db.query('SELECT * FROM users WHERE "hash"=$1', [hash], function(
                dberr,
                dbres
            ) {
                if (!dberr) {
                    db.query(
                        'UPDATE users SET "username"=$1 WHERE "hash"=$2 ', [username, hash],
                        function(dberr, dbres) {
                            if (!dberr) res.sendStatus(200);
                            else res.sendStatus(500);
                        }
    ...
    app.post("/logout", (req, res) => {
        ...
        if (hash) {
            db.query(
                'UPDATE users SET "hash"=NULL WHERE "hash"=$1', [hash],
                function(dberr, dbres) {
                    if (!dberr) res.cookie("hash", null).redirect("/");
                    else {
                        console.log(dberr);
                        res.sendStatus(500);
                    }
    ...
    function getRandomExcuse(lang, cb) {
        db.query(
            'SELECT * FROM excuses WHERE "lang"=$1 AND "verified"=true ORDER BY RANDOM() LIMIT 1', [lang],
            function(dberr, dbres) {
                if (!dberr) {
                    if (dbres.rowCount > 0) {
    ...
    function getOffsetExcuses(cb, offset, count) {
        db.query(
            'SELECT * FROM excuses ORDER BY "verified" OFFSET $1 ROWS FETCH FIRST $2 ROWS ONLY', [offset, count],
            function(dberr, dbres) {
                if (!dberr) {
                    if (dbres.rowCount > 0) {
                        cb(dbres.rows);
    ...
    function getOffsetExcusesOwn(cb, offset, count, owner) {
        db.query(
            'SELECT * FROM excuses WHERE "added_by"=$1 ORDER BY "verified" OFFSET $2 ROWS FETCH FIRST $3 ROWS ONLY', [owner, offset, count],
            function(dberr, dbres) {
                if (!dberr) {
                    if (dbres.rowCount > 0) {
                        cb(dbres.rows);
    ...
    function getOffsetProfiles(cb, offset, count) {
        db.query(
            'SELECT * FROM users ORDER BY "ID" OFFSET $1 ROWS FETCH FIRST $2 ROWS ONLY', [offset, count],
            function(dberr, dbres) {
                if (!dberr) {
                    if (dbres.rowCount > 0) {
                        cb(dbres.rows);
    ...
    function checkIfLoggedIn(req, res, cb) {
        if (req.cookies && req.cookies.hash) {
            db.query(
                'SELECT * FROM users WHERE "hash"=$1', [req.cookies.hash],
                function(dberr, dbres) {
                    if (!dberr && dbres.rowCount == 1) cb(dbres.rows[0]);
                    else if (!dberr) {
